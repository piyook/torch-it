...
if docker-compose build --pull --no-cache >> "$LOG_FILE" 2>&1; then
    ...
    warn "Docker build encountered issues - check $LOG_FILE for details"
    info "Attempting to continue with startup anyway..."
fi
...
if docker compose up -d >> "$LOG_FILE" 2>&1; then
    ...
    warn "Failed to start Docker services - check $LOG_FILE for details"
    info "You may need to start services manually with 'docker compose up -d'"
fi
